...
func main() {
	...
	http.HandleFunc("/healthcheck", func(w http.ResponseWriter, r *http.Request) {
		// Manually trigger the health check when the /healthcheck endpoint is accessed
		healthcheck()
		fmt.Fprint(w, "Health check triggered.")
	})

	// Start the HTTP server for the health check endpoint
	go func() {
		server := &http.Server{
			Addr:    ":9000",              // Set the server address and port for health check
